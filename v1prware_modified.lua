...
------------------------------------------------------------------------
-- player / world connection binding
------------------------------------------------------------------------
...
------------------------------------------------------------------------
------------------------------------------------------------------------
-- TAB: DUSEKKAR (Silent Aim)
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabDusekkar = win:Tab({ Title = "Dusekkar", Icon = "crosshair" })
...
do
    ...
    local dk_originalCB    = nil
    local dk_unloaded      = false

    local dk_AIM_OFFSET    = 0.0
    local dk_PREDICTION    = 0.15
    ...
    local function dk_getKillerVelocity(hrp)
        local now  = tick()
        local pos  = hrp.Position
        local data = dk_killerMotionData[hrp]
        if not data then
            dk_killerMotionData[hrp] = { lastPos = pos, lastTime = now, velocity = Vector3.zero }
            return Vector3.zero
        end
        local dt = now - data.lastTime
        if dt <= 0 then return data.velocity end
    ...
    -- ── ballistic camera CF solver ────────────────────────────────────
    local function dk_buildCamCF(myHRP, killerHRP)
        local hum      = myHRP.Parent and myHRP.Parent:FindFirstChildOfClass("Humanoid")
        local hipH     = hum and hum.HipHeight or 1.35
        local v238     = (hipH + myHRP.Size.Y / 2) / 2
        local spawnPos = myHRP.CFrame.Position + Vector3.new(0, v238, 0)
        ...
        return CFrame.lookAt(dk_Camera.CFrame.Position, target)
    end

    -- ── patch / unpatch ───────────────────────────────────────────────
    local function dk_getLocalChar()
        return dk_lp.Character
    end

    local function dk_applyPatch(char)
        if dk_patched or not char or not dk_NetworkRF then return end
        if type(getcallbackvalue) == "function" then
            pcall(function() dk_originalCB = getcallbackvalue(dk_NetworkRF, "OnClientInvoke") end)
        end
        dk_NetworkRF.OnClientInvoke = function(reqName, ...)
            if reqName == "GetCameraCF" and dk_enabled then
                local ch    = dk_lp.Character
                local myHRP = ch and ch:FindFirstChild("HumanoidRootPart")
                if myHRP then
                    local killer    = dk_getNearestKiller(myHRP.Position)
                    local killerHRP = killer and killer:FindFirstChild("HumanoidRootPart")
                    if killerHRP then
                        local ok, cf = pcall(dk_buildCamCF, myHRP, killerHRP)
                        if ok and cf then return cf end
                    end
                end
            end
            if dk_originalCB then return dk_originalCB(reqName, ...) end
        end
        dk_lp:SetAttribute("Device", "Mobile")
        dk_patched = true
    end

    local function dk_removePatch()
        if not dk_patched then return end
        pcall(function() if dk_NetworkRF then dk_NetworkRF.OnClientInvoke = dk_originalCB end end)
        pcall(function() dk_lp:SetAttribute("Device", nil) end)
        dk_originalCB = nil
        dk_patched    = false
    end

    -- ── __namecall hook — intercepts FireServer to redirect aim ───────
    local dk_hookConn   = nil
    local dk_origNC     = nil

    local function dk_startHook()
        if dk_hookConn then return end
        if not dk_RemoteEvent then return end
        dk_origNC = hookmetamethod(game, "__namecall", function(self, ...)
            local method = getnamecallmethod()
            if method == "FireServer" and self == dk_RemoteEvent then
                local args = { ... }
                if args[1] == "UseActorAbility"
                    and type(args[2]) == "table"
                    and typeof(args[2][1]) == "buffer"
                    and dk_enabled then
                    -- redirect camera CF so server computes aim toward nearest killer
                    local ch    = dk_lp.Character
                    local myHRP = ch and ch:FindFirstChild("HumanoidRootPart")
                    if myHRP then
                        local killer    = dk_getNearestKiller(myHRP.Position)
                        ...
                        if killerHRP then
                            local ok, cf = pcall(dk_buildCamCF, myHRP, killerHRP)
                            if ok and cf then dk_Camera.CFrame = cf end
                        end
                    end
                end
            end
            return dk_origNC(self, ...)
        end)
        dk_hookConn = true
    end

    local function dk_stopHook()
        -- unhook by restoring original; hookmetamethod stacks so we just nil the flag
        dk_hookConn = nil
    end
    ...
    task.spawn(function()
        ...
        while not dk_unloaded do
            task.wait(0.5)
            local cur = dk_getLocalChar()
            if cur ~= lastChar then
                if lastChar ~= nil then
                    dk_patched = false; dk_originalCB = nil
                    dk_killerMotionData = {}
                end
                lastChar = cur
    ...
    -- ── UI ────────────────────────────────────────────────────────────
    local dkMain = tabDusekkar:Section({ Title = "Dusekkar Silent Aim", Opened = true })

    dkMain:Toggle({
        Title = "Enable Silent Aim", Type = "Checkbox", Default = false,
        Callback = function(on)
            dk_enabled = on
            if on then
                local char = dk_getLocalChar()
                if char and not dk_patched then dk_applyPatch(char) end
                dk_startHook()
            else
                dk_stopHook()
            end
    ...
    dkMain:Slider({
        Title = "Aim Offset (Y)", Step = 0.1,
        Value = { Min = -5.0, Max = 5.0, Default = dk_AIM_OFFSET },
        Callback = function(v) dk_AIM_OFFSET = v end
    })

    dkMain:Slider({
        Title = "Prediction", Step = 0.01,
        Value = { Min = 0.0, Max = 1.0, Default = dk_PREDICTION },
