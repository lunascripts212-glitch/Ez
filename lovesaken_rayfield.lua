...
if not Rayfield then
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",
            { Title = "Error", Text = "Failed to load Rayfield", Duration = 5 })
    end)
    return
end
...
-- Rayfield handles its own UI toggle internally (RightShift by default)
...
do
    ...
    function RCT.fire()
        ...
        if now - _lastCharge < CHARGE_CD then
            pcall(function()
                Rayfield:Notify({Title="RCT",Content="Cooldown: "..math.ceil(CHARGE_CD-(now-_lastCharge)).."s",Duration=2})
            end)
            return
        end
        _lastCharge = now
...
-- ==================== CREATE TABS ====================
-- yield one frame so Rayfield window is fully ready
task.defer(function()
pcall(function() Tabs.Combat  = Window:CreateTab("Combat", nil)   end)
pcall(function() Tabs.Visuals = Window:CreateTab("Visuals", nil)     end)
pcall(function() Tabs.Player  = Window:CreateTab("Player", nil)    end)
pcall(function() Tabs.World   = Window:CreateTab("World", nil)   end)
pcall(function() Tabs.Music   = Window:CreateTab("Music", nil)   end)
pcall(function() Tabs.Elliot  = Window:CreateTab("Elliot", nil)   end)
pcall(function() Tabs.JaneDoe = Window:CreateTab("Jane Doe", nil)     end)
pcall(function() Tabs.Utility = Window:CreateTab("Utility", nil)  end)
pcall(function() Tabs.Config  = Window:CreateTab("Config", nil) end)

-- ==================== COMBAT TAB UI ====================
do
    local tab = Tabs.Combat
    pcall(function()
        local sAB = tab:CreateSection("Auto Block")

        local autoBlockToggle = sAB:CreateToggle({
            Name = "Auto Block (Audio)",
            CurrentValue = S.autoBlockAudioOn,
            Callback = function(v)
                pcall(function()
        ...
        sAB:CreateDropdown({Name="Block Type",Options={"Block","Charge","7n7 Clone"},CurrentOption=S.autoblocktype,Callback=function(v)
            pcall(function() S.autoblocktype=v; set("autoblocktype",v) end)
        end})
        sAB:CreateSlider({Name="Block Delay",Range={0,0.5},Increment=0.01,CurrentValue=S.blockdelay,Callback=function(v)
            S.blockdelay=(tonumber(v) or S.blockdelay); set("blockdelay",S.blockdelay)
        end})
        sAB:CreateSlider({Name="Detection Range",Range={5,50},Increment=1,CurrentValue=S.detectionRange,Callback=function(v)
            pcall(function() S.detectionRange=(tonumber(v) or S.detectionRange); set("detectionRange",S.detectionRange) end)
        end})
        sAB:CreateToggle({Name="Facing Check",CurrentValue=S.facingCheckEnabled,Callback=function(v)
            pcall(function() S.facingCheckEnabled=v; set("facingCheckEnabled",v) end)
        end})
        sAB:CreateToggle({Name="Double Punch Tech",CurrentValue=S.doubleblocktech,Callback=function(v)
            pcall(function() S.doubleblocktech=v; set("doubleblocktech",v) end)
        end})
        sAB:CreateToggle({Name="Anti-Bait",CurrentValue=S.antiBaitEnabled,Callback=function(v)
            pcall(function() S.antiBaitEnabled=v; set("antiBaitEnabled",v) end)
        end})
        sAB:CreateSlider({Name="Block Miss Chance",Range={0,100},Increment=1,CurrentValue=S.abMissChance,Callback=function(v)
            pcall(function() S.abMissChance=(tonumber(v) or S.abMissChance); set("abMissChance",S.abMissChance) end)
        end})
    end)
    pcall(function()
        local sHDT = tab:CreateSection("HDT Tech")
        sHDT:CreateToggle({Name="Hitbox Dragging",CurrentValue=S.hitboxDraggingTech,Callback=function(v)
            pcall(function() S.hitboxDraggingTech=v; set("hitboxDraggingTech",v) end)
        end})
        sHDT:CreateSlider({Name="HDT Speed",Range={1,30},Increment=0.5,CurrentValue=S.Dspeed,Callback=function(v)
            pcall(function() S.Dspeed=(tonumber(v) or S.Dspeed); set("Dspeed",S.Dspeed) end)
        end})
        sHDT:CreateSlider({Name="HDT Delay",Range={0,0.5},Increment=0.01,CurrentValue=S.Ddelay,Callback=function(v)
            pcall(function() S.Ddelay=(tonumber(v) or S.Ddelay); set("Ddelay",S.Ddelay) end)
        end})
        sHDT:CreateSlider({Name="Rotate Delay",Range={0,0.5},Increment=0.01,CurrentValue=S.rotateDelay,Callback=function(v)
            pcall(function() S.rotateDelay=(tonumber(v) or S.rotateDelay); set("rotateDelay",S.rotateDelay) end)
        end})
        sHDT:CreateSlider({Name="HDT Miss Chance",Range={0,100},Increment=1,CurrentValue=S.hdtMissChance,Callback=function(v)
            pcall(function() S.hdtMissChance=(tonumber(v) or S.hdtMissChance); set("hdtMissChance",S.hdtMissChance) end)
        end})
        sHDT:CreateDropdown({Name="HDT Mode",Options={"180_TURN","LEFT_SPIN","RIGHT_SPIN"},CurrentOption=S.hdtMode,Callback=function(v)
            pcall(function() S.hdtMode=v; set("hdtMode",v) end)
        end})
    end)
    pcall(function()
        local sRCT = tab:CreateSection("Reverse Charge Tech (RCT)")
        sRCT:CreateToggle({Name="Enable RCT",CurrentValue=S.rctEnabled,Callback=function(v)
            pcall(function()
                S.rctEnabled=v; set("rctEnabled",v)
                Rayfield:Notify({Title="RCT",Content=v and"Active"or"Disabled",Duration=2})
            end)
        end})
        sRCT:CreateButton({Name="Fire Charge + Flick",Callback=function()
            pcall(function()
                if not S.rctEnabled then Rayfield:Notify({Title="RCT",Content="Enable RCT first!",Duration=2}); return end
                RCT.fire()
            end)
        end})
        sRCT:CreateDropdown({Name="Flick Direction",Options={"Right","Left","Back","Auto"},CurrentOption=S.rctFlickDir,Callback=function(v)
            pcall(function() S.rctFlickDir=v; set("rctFlickDir",v) end)
        end})
        sRCT:CreateToggle({Name="Auto Ledge",CurrentValue=S.rctAutoLedge,Callback=function(v)
            pcall(function() S.rctAutoLedge=v; set("rctAutoLedge",v) end)
        end})
        sRCT:CreateSlider({Name="Flick Angle",Range={45,180},Increment=5,CurrentValue=S.rctFlickAngle,Callback=function(v)
            pcall(function() S.rctFlickAngle=(tonumber(v) or S.rctFlickAngle); set("rctFlickAngle",S.rctFlickAngle) end)
        end})
        sRCT:CreateSlider({Name="Flick Delay",Range={0,0.3},Increment=0.01,CurrentValue=S.rctFlickDelay,Callback=function(v)
            pcall(function() S.rctFlickDelay=(tonumber(v) or S.rctFlickDelay); set("rctFlickDelay",S.rctFlickDelay) end)
        end})
        sRCT:CreateSlider({Name="Flick Speed",Range={0.02,0.2},Increment=0.01,CurrentValue=S.rctFlickSpeed,Callback=function(v)
            pcall(function() S.rctFlickSpeed=(tonumber(v) or S.rctFlickSpeed); set("rctFlickSpeed",S.rctFlickSpeed) end)
        end})
        sRCT:CreateSlider({Name="RCT Miss Chance",Range={0,100},Increment=1,CurrentValue=S.rctMissChance,Callback=function(v)
            pcall(function() S.rctMissChance=(tonumber(v) or S.rctMissChance); set("rctMissChance",S.rctMissChance) end)
        end})
    end)
    pcall(function()
        local sPunch = tab:CreateSection("Auto Punch")
        sPunch:CreateToggle({Name="Auto Punch",CurrentValue=S.autoPunchOn,Callback=function(v)
            pcall(function() S.autoPunchOn=v; set("autoPunchOn",v) end)
        end})
        local sLock = tab:CreateSection("Lock (Survivor Side)")
        sLock:CreateToggle({Name="Character Lock",CurrentValue=S.characterLockOn,Callback=function(v)
            pcall(function() S.characterLockOn=v; set("characterLockOn",v) end)
        end})
        sLock:CreateSlider({Name="Max Distance",Range={5,100},Increment=5,CurrentValue=S.lockMaxDistance,Callback=function(v)
            pcall(function() S.lockMaxDistance=(tonumber(v) or S.lockMaxDistance); set("lockMaxDistance",S.lockMaxDistance) end)
        end})
        sLock:CreateSlider({Name="Prediction",Range={0,15},Increment=0.5,CurrentValue=S.predictionValue,Callback=function(v)
            pcall(function() S.predictionValue=(tonumber(v) or S.predictionValue); set("predictionValue",S.predictionValue) end)
        end})
        local sOV = tab:CreateSection("Obsidian Vision")
        sOV:CreateToggle({Name="Detection Circles",CurrentValue=S.killerCirclesVisible,Callback=function(v)
            pcall(function() S.killerCirclesVisible=v; set("killerCirclesVisible",v); OV.refreshCircles() end)
        end})
        sOV:CreateToggle({Name="Facing Visual",CurrentValue=S.facingVisualOn,Callback=function(v)
...
do
    ...
    pcall(function()
        local sCrystal = tab:CreateSection("Crystal Auto-Fire + Silent Aim")

        sCrystal:CreateToggle({
            Name = "Enable Crystal Auto-Fire",
            Value = false,
            Callback = function(on)
                ...
                Rayfield:Notify({Title="Jane Doe Crystal", Content=on and "Enabled" or "Disabled", Duration=2})
            end
        })
    ...
    pcall(function()
        local sAxe = tab:CreateSection("Axe Lock")

        sAxe:CreateParagraph({
            Title   = "How it works",
            Content = "Intercepts your Axe FireServer call and face-locks your character toward the nearest killer for the set duration. Camera never moves."
        })
    ...
    pcall(function()
        local sCtrl = tab:CreateSection("Control")
        sCtrl:CreateButton({Name="Unload Jane Doe",
            Callback = function()
                JaneDoe.unload()
                Rayfield:Notify({Title="Jane Doe", Content="Unloaded successfully.", Duration=3})
            end
        })
    end)
end
...
do
    ...
    pcall(function()
        local sElliot = tab:CreateSection("Elliot Aimbot")

        sElliot:CreateToggle({Name="Elliot Aimbot", CurrentValue=S.elliotAimbotEnabled, Callback=function(v)
            S.elliotAimbotEnabled = v; set("elliotAimbotEnabled", v)
            if v then ElliotAimbot.start(); Rayfield:Notify({Title="Elliot Aimbot", Content="Enabled!", Duration=2})
            else ElliotAimbot.stop(); Rayfield:Notify({Title="Elliot Aimbot", Content="Disabled", Duration=2}) end
        end})

        sElliot:CreateToggle({Name="Require Throw Animation", CurrentValue=S.elliotRequireAnimation, Callback=function(v)
            S.elliotRequireAnimation = v; set("elliotRequireAnimation", v)
        end})

        sElliot:CreateSlider({Name="Prediction Studs",Range={0,50},Increment=1,CurrentValue=S.elliotPrediction,Callback=function(v)
            S.elliotPrediction = tonumber(v) or 5; set("elliotPrediction", S.elliotPrediction)
        end})

        sElliot:CreateSlider({Name="Velocity Threshold",Range={0,50},Increment=1,CurrentValue=S.elliotVelocityThreshold,Callback=function(v)
            S.elliotVelocityThreshold = tonumber(v) or 16; set("elliotVelocityThreshold", S.elliotVelocityThreshold)
        end})

        sElliot:CreateSlider({Name="Aim Duration (seconds)",Range={0.1,2},Increment=0.1,CurrentValue=S.elliotAimDuration,Callback=function(v)
            S.elliotAimDuration = tonumber(v) or 0.5; set("elliotAimDuration", S.elliotAimDuration)
        end})

        sElliot:CreateSlider({Name="Pizza Throw Force",Range={50,150},Increment=5,CurrentValue=80,Callback=function(v)
            ElliotAimbot.setPizzaForce(v)
        end})

        sElliot:CreateSlider({Name="Arc Segments",Range={20,100},Increment=5,CurrentValue=50,Callback=function(v)
            ElliotAimbot.setArcSegments(v)
        end})

        sElliot:CreateToggle({Name="Show Pizza Arc", CurrentValue=false, Callback=function(v)
            ElliotAimbot.setShowArc(v)
            Rayfield:Notify({Title="Pizza Arc", Content=v and "Enabled!" or "Disabled", Duration=2})
        end})

        local pingDisplay = sElliot:CreateParagraph({Title="Network Ping", Content="Calculating..."})
        task.spawn(function()
            while task.wait(1) do
                ...
                sElliot:CreateButton({Name="Manual Rescan",Callback=function()
            if S.elliotAimbotEnabled then
                ElliotAimbot.stop(); task.wait(0.2); ElliotAimbot.start()
                Rayfield:Notify({Title="Elliot Aimbot", Content="Rescanned!", Duration=2})
            end
        end})
    end)
end
...
do
    ...
    pcall(function()
        local sChat = tab:CreateSection("Chat Logger")

        sChat:CreateToggle({Name="Enable Chat Logger", CurrentValue=S.chatLogEnabled, Callback=function(v)
            pcall(function()
                S.chatLogEnabled = v
        ...
        sChat:CreateButton({Name="Show/Hide Chat Window",Callback=function()
            pcall(function() ChatLogger.toggle() end)
        end})

        sChat:CreateButton({Name="Clear Chat Log",Callback=function()
            pcall(function()
                ChatLogger.clear()
                Rayfield:Notify({Title="Chat Log", Content="Cleared!", Duration=2})
            end)
        end})

        sChat:CreateParagraph({
            Title = "Chat Window Info",
            Content = "Positioned center-left. Drag title bar to move. Click ✕ to hide."
        })

        
        local sAntiTaph = tab:CreateSection("Anti-Taph")
        sAntiTaph:CreateToggle({Name="Remove Blindness/Effects", CurrentValue=S.antiTaphEnabled, Callback=function(v)
            pcall(function()
                S.antiTaphEnabled = v
                set("antiTaphEnabled", v)
                if v then
                    AntiTaph.apply()
                    Rayfield:Notify({Title="Anti-Taph", Content="Blindness protection active!", Duration=2})
                else
                    AntiTaph.remove()
                end
            end)
        ...
        local sSpeed = tab:CreateSection("Speed Hack")
        sSpeed:CreateToggle({Name="Enable Speed Hack", CurrentValue=S.speedHackEnabled, Callback=function(v)
            pcall(function()
        ...
        sSpeed:CreateSlider({Name="Walk Speed",Range={16,120},Increment=1,CurrentValue=S.speedHackValue,Callback=function(v)
            pcall(function()
        ...
        local sMobile = tab:CreateSection("Mobile Quick Toggle")
        sMobile:CreateToggle({Name="Show Quick AB Toggle Button", CurrentValue=S.mobileQuickToggle, Callback=function(v)
            pcall(function()
        ...
        sMobile:CreateParagraph({
            Title = "Info",
            Content = "Creates a draggable button on screen to quickly toggle Auto Block on/off.\nTapping it also syncs the main Combat tab toggle."
        })
    end)
end
...
do
    ...
    pcall(function()
        local sp = tab:CreateSection("Player ESP")
        sp:CreateToggle({Name="Killers",CurrentValue=ESP.Enabled.Killers,Callback=function(v)
            pcall(function() ESP.Enabled.Killers=v; set("espKillersEnabled",v); if v then ESP.applyKillers() else ESP.removeKillers() end end)
        end})
        sp:CreateToggle({Name="Survivors",CurrentValue=ESP.Enabled.Survivors,Callback=function(v)
        ...
        local sw = tab:CreateSection("World ESP")
        sw:CreateToggle({Name="Generators",CurrentValue=ESP.Enabled.Generators,Callback=function(v)
        ...
        sw:CreateToggle({Name="Items",CurrentValue=ESP.Enabled.Items,Callback=function(v)
            pcall(function() ESP.Enabled.Items=v; set("espItemsEnabled",v); if v then ESP.applyItems() else ESP.removeItems() end end)
        end})
    end)
    pcall(function()
        local sb = tab:CreateSection("Builderman ESP")
        sb:CreateToggle({Name="Sentry",CurrentValue=ESP.Enabled.Sentries,Callback=function(v)
            pcall(function() ESP.Enabled.Sentries=v; set("espSentryEnabled",v); if v then ESP.applySentries() else ESP.removeSentries() end end)
        end})
        sb:CreateToggle({Name="Dispenser",CurrentValue=ESP.Enabled.Dispensers,Callback=function(v)
        ...
        local sf = tab:CreateSection("Fake Generator ESP")
        sf:CreateToggle({Name="Fake Generators",CurrentValue=ESP.Enabled.FakeGens,Callback=function(v)
    ...
    pcall(function()
        local sm = tab:CreateSection("Minion ESP")
        sm:CreateToggle({Name="c00lkidd Minions",CurrentValue=ESP.Enabled.MinionC00l,Callback=function(v)
        ...
        sm:CreateToggle({Name="1x1x1x1 Minions",CurrentValue=ESP.Enabled.Minion1x1,Callback=function(v)
        ...
        sm:CreateButton({Name="Force Rescan Minions",Callback=function()
            pcall(function()
                ...
                Rayfield:Notify({Title="Minion ESP",Content="Rescanned!",Duration=2})
            end)
        end})
    end)
    pcall(function()
        local st = tab:CreateSection("Taph Trap ESP")
        st:CreateToggle({Name="Tripwire ESP",CurrentValue=TaphESP.tripwire.enabled,Callback=function(v)
        ...
        st:CreateToggle({Name="Tripmine ESP",CurrentValue=TaphESP.tripmine.enabled,Callback=function(v)
        ...
        st:CreateButton({Name="Force Rescan",Callback=function()
            pcall(function()
                TaphESP.reset(); safeSpawn(TaphESP.setup); safeSpawn(TaphESP.refresh)
                Rayfield:Notify({Title="Taph",Content="Rescanning...",Duration=2})
            end)
        end})
        local ss = tab:CreateSection("Style Settings")
        ss:CreateDropdown({Name="ESP Style",Options={"Glow","Outline","Minimal"},CurrentOption=ESP.Settings.Style,Callback=function(v)
            pcall(function() ESP.Settings.Style=v; set("espStyle",v); ESP.refresh() end)
        end})
        ss:CreateToggle({Name="Show Distance",CurrentValue=ESP.Settings.ShowDistance,Callback=function(v)
            pcall(function() ESP.Settings.ShowDistance=v; set("espShowDistance",v) end)
        end})
        ss:CreateToggle({Name="Show Health",CurrentValue=ESP.Settings.ShowHealth,Callback=function(v)
            pcall(function() ESP.Settings.ShowHealth=v; set("espShowHealth",v) end)
        end})
        ss:CreateSlider({Name="Transparency",Range={0,1},Increment=0.05,CurrentValue=ESP.Settings.Transparency,Callback=function(v)
            pcall(function() ESP.Settings.Transparency=v; set("espTransparency",v); ESP.refresh() end)
        end})
        ss:CreateButton({Name="Force Refresh ESP",Callback=function()
            pcall(function()
                ...
                if ESP.Enabled.Dispensers then ESP.applyDispensers() end
                if ESP.Enabled.FakeGens   then ESP.applyFakeGens()   end
                Rayfield:Notify({Title="ESP",Content="Refreshed!",Duration=2})
            end)
        end})
    end)
end
...
do
    ...
    pcall(function()
        local ss = tab:CreateSection("Stamina Control")
        ss:CreateToggle({Name="Custom Stamina",CurrentValue=S.staminaCustomEnabled,Callback=function(v)
            pcall(function() S.staminaCustomEnabled=v; set("staminaCustomEnabled",v); if v then pcall(Stamina.apply) end end)
        end})
        ss:CreateToggle({Name="Disable Stamina Loss",CurrentValue=S.staminaLossDisabled,Callback=function(v)
        ...
        ss:CreateSlider({Name="Loss Rate",Range={0,50},Increment=1,CurrentValue=S.staminaLossValue,Callback=function(v)
        ...
        ss:CreateSlider({Name="Gain Rate",Range={0,50},Increment=1,CurrentValue=S.staminaGainValue,Callback=function(v)
        ...
        ss:CreateSlider({Name="Max Stamina",Range={50,500},Increment=10,CurrentValue=S.staminaMaxValue,Callback=function(v)
...
do
    ...
    pcall(function()
        local sg = tab:CreateSection("Generator Solver")
        sg:CreateToggle({Name="Auto Solve",CurrentValue=S.genFlowSolverEnabled,Callback=function(v)
            pcall(function() S.genFlowSolverEnabled=v; set("genFlowSolverEnabled",v) end)
        end})
        sg:CreateSlider({Name="Node Speed",Range={0.01,0.5},Increment=0.01,CurrentValue=S.genFlowNodeDelay,Callback=function(v)
            pcall(function() S.genFlowNodeDelay=(tonumber(v) or S.genFlowNodeDelay); set("genFlowNodeDelay",S.genFlowNodeDelay) end)
        end})
        sg:CreateSlider({Name="Line Delay",Range={0,1},Increment=0.01,CurrentValue=S.genFlowLineDelay,Callback=function(v)
...
do
    ...
    pcall(function()
        ...
        table.sort(opts)
        local sl = tab:CreateSection("LMS Custom Music")
        sl:CreateToggle({Name="Auto-Play LMS Music",CurrentValue=S.lmsAutoPlay,Callback=function(v)
            pcall(function()
                S.lmsAutoPlay=v; set("lmsAutoPlayEnabled",v)
                if v then LMS.start(); Rayfield:Notify({Title="LMS",Content="Monitor started",Duration=2})
                else LMS.stop(); Rayfield:Notify({Title="LMS",Content="Disabled",Duration=2}) end
            end)
        end})
        sl:CreateDropdown({Name="Select Song",Options=opts,CurrentOption=S.lmsSelectedSong,Callback=function(v)
            pcall(function() S.lmsSelectedSong=v; set("selectedLMSSong",v); task.spawn(function() LMS.download(v) end) end)
        end})
        sl:CreateButton({Name="Play Now",Callback=function()
            pcall(function()
                if LMS.playNow() then Rayfield:Notify({Title="LMS",Content="Now playing: "..S.lmsSelectedSong,Duration=3})
                else Rayfield:Notify({Title="Error",Content="Sound not found",Duration=3}) end
            end)
        end})
        sl:CreateButton({Name="Reset",Callback=function()
            pcall(function() LMS.reset(); Rayfield:Notify({Title="LMS",Content="Reset",Duration=2}) end)
        end})
    end)
end
...
do
    ...
    pcall(function()
        local st = tab:CreateSection("Theme")
        st:CreateDropdown({
            Title="Theme",
            Values={"Dark","Light","Rose","Plant","Rainbow","Midnight","Violet",
                "CottonCandy","MonokaiPro","Indigo","Sky","Crimson","Amber","Emerald","Red"},
        })
        local sc = tab:CreateSection("Configuration")
        sc:CreateButton({Name="Save Config",Callback=function()
            pcall(function() Config.save(); Rayfield:Notify({Title="Saved",Duration=2}) end)
        end})
        sc:CreateButton({Name="Load Config",Callback=function()
            pcall(function()
                if Config.load() then
                    Rayfield:Notify({Title="Loaded",Duration=2})
                    ...
                    Rayfield:Notify({Title="No config found",Duration=2})
                end
            end)
        end})
        sc:CreateButton({Name="Reset Config",Callback=function()
            pcall(function()
                delfile(Config.configFile); Config.data={}
                Rayfield:Notify({Title="Config reset",Duration=2})
            end)
        end})
    end)
end
end) -- end task.defer for tab creation
